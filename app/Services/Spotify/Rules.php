...
final class Rules {
 ...
 public static function months(mixed $value):int {$n=filter_var($value,FILTER_VALIDATE_INT,['options'=>['min_range'=>1,'max_range'=>120]]);if($n===false)throw new HttpException(422,'Selecciona entre 1 y 120 meses.');return $n;}
 public static function addMonths(string $date,int $months):string {$day=new \DateTimeImmutable(self::date($date));$next=$day->modify('first day of this month')->modify('+'.$months.' months');return $next->setDate((int)$next->format('Y'),(int)$next->format('m'),min((int)$day->format('d'),(int)$next->format('t')))->format('Y-m-d');}
