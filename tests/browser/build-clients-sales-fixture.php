...
window.fetch=async(url,options={})=>{
 ...
 if(u.pathname.endsWith('clientes.php')){
  if(options.body?.get('action')==='import'){Object.assign(data,{message:'1 clientes importados.',imported:1,errors:[{line:3,name:'Duplicado',phone:'+51987654321',values:['Duplicado','+51987654321'],reason:'El celular ya existe.'}]});}
  ...
  else if(u.searchParams.get('action')==='information')Object.assign(data,{client:clients[0],rows:[{service:'Spotify',email:'cuenta@example.test',beneficiary:'Mamá',start_date:'2026-09-01',end_date:'2026-10-01',days:7,status:'active',released_at:null},{service:'Spotify',email:'anterior@example.test',beneficiary:null,start_date:'2026-07-01',end_date:'2026-08-01',days:null,status:'released',released_at:'2026-07-20'}],total:2,page:1,pages:1});
...
window.addEventListener('load',async()=>{
 ...
 try{
  ...
  document.querySelector('#clientsTable [data-client-info]').click();await wait();const info=document.getElementById('clientServicesTable');if(info.tBodies[0].rows.length!==2||!info.textContent.includes('Liberado')||!info.textContent.includes('20/07/2026')||info.textContent.includes('Contraseña'))throw Error('Información incompleta');bootstrap.Modal.getInstance(document.getElementById('clientInfoModal')).hide();await wait();
  document.querySelector('#clientsTable [data-client-edit]').click();await wait();document.getElementById('clientPhone').value='+12025550123';document.getElementById('clientName').value='Nombre nuevo';document.getElementById('clientForm').requestSubmit();await wait();
  ...
  const report=document.getElementById('clientsImportReport');if(!document.getElementById('clientImportModal').contains(report)||!document.getElementById('clientImportModal').classList.contains('show')||report.querySelector('table,ul')||!report.textContent.includes('Se rechazaron 1 registros'))throw Error('Resultado no está compacto en modal');let csvText='';const originalURL=URL.createObjectURL;URL.createObjectURL=blob=>{blob.text().then(text=>csvText=text);return originalURL(blob);};report.querySelector('button').click();await wait();if(!csvText.includes('El celular ya existe.')||!csvText.includes('Duplicado'))throw Error('Descarga sin detalle');
