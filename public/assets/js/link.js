...
function iniciarLink() {
 ...
 function setBusy(value){busy=value;$('linkBusy').hidden=!value;form.querySelectorAll('button').forEach(b=>b.disabled=value);modalElement.querySelector('.btn-close').disabled=value;form.querySelectorAll('input,select,textarea').forEach(e=>e.disabled=value||!!e.closest('[data-link-panel][hidden]'));if(!value && mode==='move')moveFields();}
 ...
 root.addEventListener('click',async event=>{const button=event.target.closest('[data-link-action]');if(!button||busy)return;const action=button.dataset.linkAction,row=rows.find(r=>r.id===Number(button.dataset.id));
  ...
  if(action==='edit'){open('edit','Editar cuenta');$('linkClearTokens').hidden=false;setBusy(true);try{const data=await request('details',null,{id:row.id});for(const key of ['id','revision','credentials','external_id','secure'])form.elements[key].value=data[key];}catch(e){fail(e);}finally{setBusy(false);}return;}
 ...
 form.addEventListener('submit',async event=>{event.preventDefault();if(busy)return;$('linkModalError').hidden=true;const data=mode==='move'?moveData():new FormData(form);if(mode==='import')$('linkImportResult').replaceChildren();setBusy(true);
  try{
   ...
   if(mode==='import'){report=await request('import',data);$('linkImportResult').replaceChildren();const p=document.createElement('p');p.textContent='Se importaron '+report.loaded+' registros. Se rechazaron '+report.rejected+' registros.';$('linkImportResult').append(p);if(report.errors.length){const b=document.createElement('button');b.type='button';b.className='btn btn-sm btn-outline-primary';b.textContent='Descargar CSV de rechazos';b.onclick=()=>{const csvCell=value=>{let text=String(value??'');if(/^[=+@\-\t\r]/.test(text))text="'"+text;return '"'+text.replaceAll('"','""')+'"';};const csv='Fila,Correo,ID,secure,correo_contrasena,Motivo,Datos originales\r\n'+report.errors.map(e=>[e.row,e.email,e.data?.[0],e.data?.[1],e.data?.[2],e.reason,JSON.stringify(e.data||[])].map(csvCell).join(',')).join('\r\n');const url=URL.createObjectURL(new Blob(['\uFEFF'+csv],{type:'text/csv;charset=utf-8'}));const a=document.createElement('a');a.href=url;a.download='rechazos-cuentas.csv';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);};$('linkImportResult').append(b);} }
 ...
 $('linkClearTokens').onclick=()=>{if(busy||mode!=='edit')return;['linkExternalId','linkSecure'].forEach(id=>{$(id).value='';$(id).dispatchEvent(new Event('input',{bubbles:true}));});$('linkExternalId').focus();};
